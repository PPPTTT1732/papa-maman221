...
liveRouter.get("/live-sessions", (req, res) => {
  res.json(readDb().liveSessions || []);
});

liveRouter.post("/live-sessions/:id/reaction", (req, res) => {
  const { id } = req.params;
  const { type } = req.body;
  const db = readDb();
...
liveRouter.post("/live-sessions/:id/chat", (req, res) => {
  const { id } = req.params;
  ...
  if (idx !== undefined && idx !== -1) {
    ...
    const activeUser = user || db.studentProfile?.name || "Écolier Anonyme";
    const now = new Date();
