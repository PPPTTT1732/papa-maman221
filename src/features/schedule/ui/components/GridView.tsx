...
export function GridView({ state }: { state: any }) {
  const { schedule, gridContainerRef, scrollGrid, fitsFilters, setActivePlanCourse } = state;
                      ...
                      <td key={day} rowSpan={rowSpan} className={`p-2 border-r border-neutral-gray-150 align-top ${matchedCourse ? 'h-auto' : 'h-28'}`}>
                        {matchedCourse && (
                          <div onClick={() => match && setActivePlanCourse(matchedCourse)} style={{ height: `${rowSpan * 105}px` }} className={`p-3.5 rounded-2xl transition-all duration-305 flex flex-col justify-between group overflow-hidden border ${!match ? 'opacity-10 grayscale scale-[0.98] pointer-events-none select-none bg-neutral-gray-50/50 shadow-none border-transparent' : matchedCourse.status === 'actuel' ? 'bg-[#B3181C] text-white border-transparent shadow-md ring-4 ring-[#B3181C]/5 cursor-pointer hover:shadow-lg scale-101' : matchedCourse.type === 'CM' ? 'bg-[#FFF5F5] border-[#B3181C]/15 hover:bg-[#ffebeb] text-[#291715] cursor-pointer hover:shadow-xs' : matchedCourse.type === 'TP' ? 'bg-[#E1F7EC] border-[#10B981]/20 hover:bg-[#d0f2df] text-neutral-850 cursor-pointer hover:shadow-xs' : 'bg-blue-50/70 border-blue-200/50 hover:bg-blue-100 text-neutral-800 cursor-pointer hover:shadow-xs'}`}>
                            <div>
                              <div className="flex justify-between items-center mb-1.5"><span className={`text-[9px] font-black tracking-wider uppercase ${matchedCourse.status === 'actuel' ? 'text-white/80' : 'text-neutral-400'}`}>{matchedCourse.heureStr}</span><span className={`px-2 py-0.5 rounded text-[8px] font-black tracking-widest ${matchedCourse.status === 'actuel' ? 'bg-white/20 text-white' : 'bg-white text-[#B3181C] border border-[#B3181C]/10'}`}>{matchedCourse.type}</span></div>
                            ...
                            <div className="mt-2.5 pt-2 border-t border-neutral-200/10 flex items-center justify-between gap-1 select-none"><span className={`text-[9px] flex items-center gap-1 font-black ${matchedCourse.status === 'actuel' ? 'text-white/90' : 'text-[#B3181C]'}`}><MapPin className="h-3 w-3" />{matchedCourse.salle}</span><span className={`text-[8.5px] font-black uppercase tracking-widest group-hover:inline-block hidden ${matchedCourse.status === 'actuel' ? 'text-white/95' : 'text-[#B3181C]'}`}>Agenda &rarr;</span></div>
                          </div>
                        )}
                      </td>
                    );
