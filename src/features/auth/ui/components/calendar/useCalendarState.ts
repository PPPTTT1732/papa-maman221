...
function matchesProfessor(profNameInSession: string, currentProfNom: string): boolean {
...
export function useCalendarState() {
  ...
  const displayedSessions = rawSessions.filter(c => {
    ...
    return c.nom.toLowerCase().includes(q) || c.professeur.toLowerCase().includes(q) || c.salle.toLowerCase().includes(q);
  });
